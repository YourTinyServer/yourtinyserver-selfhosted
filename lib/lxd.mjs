...
import { DEFAULT_DISTRIBUTION, distributionByName } from "./distributions.mjs";
...
export async function listInstances(project) {
  ...
  return instances
    ...
    .map((instance) => ({
      ...
      distribution: instance.config?.["user.yts.distribution"] || DEFAULT_DISTRIBUTION,
...
export async function reinstallInstance(project, name, distributionName) {
  const distribution = distributionByName(distributionName);
  if (!distribution) throw new Error("Unsupported Linux distribution");
  const before = await getInstanceDetails(project, name);
  for (const snapshot of before.snapshots) {
    await lxc(["delete", `${name}/${snapshot.name}`, "--project", project], 180_000);
  }
  await lxc(["rebuild", distribution.alias, name, "--project", project, "--force"], 900_000);
  await lxc(["config", "set", name, `user.yts.distribution=${distribution.name}`, "--project", project]);
  const shouldRun = before.status === "running" || before.status === "frozen";
  let current = await managedInstance(project, name);
  if (shouldRun && String(current.status || "").toLowerCase() !== "running") {
    await lxc(["start", name, "--project", project], 120_000);
  }
  if (before.status === "frozen") await lxc(["pause", name, "--project", project], 60_000);
  for (let attempt = 0; attempt < 20; attempt += 1) {
    const details = await getInstanceDetails(project, name);
    if (!shouldRun || details.ipv4) return details;
...
export async function createInstance(project, profile, distributionName, requestedName) {
  ...
  if (!isManagedName(name)) throw new Error("Invalid instance name");
  await lxc([
    "launch", distribution.alias, name,
    "--project", project,
    "--profile", profile,
    "--config", "user.yts.managed=true",
    "--config", `user.yts.distribution=${distribution.name}`,
  ], 900_000);
  return name;
}
