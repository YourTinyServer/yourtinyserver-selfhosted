...
export function attachTerminalGateway(server, { origin, project, authorize }) {
  ...
  server.on("upgrade", async (request, socket, head) => {
    try {
      ...
      if (url.pathname !== "/terminal-ws" || request.headers.origin !== origin || !isManagedName(name) || !await authorize(request)) throw new Error("Forbidden");
