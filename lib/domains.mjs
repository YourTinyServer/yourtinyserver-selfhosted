...
export async function refreshInstanceDomains(project, instanceName) {
  return serialized(async () => {
    ...
    for (const route of routes) {
      ...
      await writeRoute(paths.available, tlsConfig(route.domain, instance.ipv4, route.targetPort, route.certificateName, route.targetProtocol));
    }
    await reloadNginx();
  });
}
...
export async function provisionDomain(project, instanceName, input) {
  return serialized(async () => {
    ...
    const targetProtocol = input.targetProtocol === "https" ? "https" : "http";
    ...
    const route = {
      id: randomUUID(), instanceName, domain, targetPort, targetProtocol, status: "provisioning",
...
export async function updateDomain(project, instanceName, input) {
  return serialized(async () => {
    const targetPort = Number(input.targetPort);
    const targetProtocol = input.targetProtocol === "https" ? "https" : input.targetProtocol === "http" ? "http" : null;
    if (!Number.isInteger(targetPort) || targetPort < 1 || targetPort > 65535) throw new Error("Target port must be between 1 and 65535");
    if (!targetProtocol) throw new Error("Internal protocol must be HTTP or HTTPS");
    const routes = await readStore();
    ...
    route.targetProtocol = targetProtocol;
    await writeStore(routes);
    return route;
  });
}
