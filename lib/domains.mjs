...
function proxyLocation(ipv4, port) {
  return `  location / {
...
function httpConfig(domain, ipv4, port) {
  return `server {
  listen 80;
  listen [::]:80;
  server_name ${domain};
...
${proxyLocation(ipv4, port)}
}
`;
}

function tlsConfig(domain, ipv4, port, certName) {
  return `server {
  listen 80;
  listen [::]:80;
  server_name ${domain};
...
export async function refreshInstanceDomains(project, instanceName) {
  return serialized(async () => {
    ...
    for (const route of routes) {
      ...
      await writeRoute(paths.available, tlsConfig(route.domain, instance.ipv4, route.targetPort, route.certificateName));
    }
    await reloadNginx();
  });
}
...
export async function provisionDomain(project, instanceName, input) {
  return serialized(async () => {
    ...
    try {
      ...
      await writeRoute(paths.available, httpConfig(domain, instance.ipv4, targetPort));
      ...
      await writeRoute(paths.available, tlsConfig(domain, instance.ipv4, targetPort, certName));
      await reloadNginx();
      route.status = "active";
